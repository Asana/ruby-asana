  ...
  module Resources
    ...
    class Team < Resource
      ...
      def add_user(user: required("user"), options: {}, **data)
        ...
        User.new(parse(client.post("/teams/%s/addUser", body: with_params, options: options)).first, client: client)
      end
      ...
      def remove_user(user: required("user"), options: {}, **data)
        with_params = data.merge(user: user).reject { |_,v| v.nil? || Array(v).empty? }
        client.post("/teams/%s/removeUser", body: with_params, options: options) && true
      end
